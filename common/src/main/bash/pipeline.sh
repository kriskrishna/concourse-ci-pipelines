...
function deployRabbitMqToCf() {
    local rabbitMqAppName="${1:-geodoc-rabbitmq}"
...
function deployMySqlToCF() {
    local mysqlAppName="${1:-geodoc-mysql}"
    echo "Waiting for MySQL to start"
    # create MySQL
    APP_NAME="${mysqlAppName}"
    (cf s | grep "${APP_NAME}" && echo "found ${APP_NAME}") ||
        (cf cs cleardb spark "${APP_NAME}" && echo "Started MySQL") ||
        (cf cs p-mysql 100mb "${APP_NAME}" && echo "Started MySQL for PCF Dev")
}
...
function deployAppWithName() {
    ...
    if [[ ! -z "${manifestOption}" ]]; then
        cf push "${lowerCaseAppName}" -m 1024m -i 1 -p "${OUTPUT_FOLDER}/${jarName}.jar" -n "${hostname}" --no-start ${manifestOption}
    else
        cf push "${lowerCaseAppName}" -p "${OUTPUT_FOLDER}/${jarName}.jar" -n "${hostname}" --no-start -b java_buildpack_offline_latest
    fi
...
function downloadJar() {
    ...
    local artifactId="${4}"
    local version="${5}"
    local insecureSsl="${6:-false}"
    local verifySslFlag=""
    if [ "$insecureSsl" = true ]; then
        verifySslFlag="-k"
    fi
    ...
    if [[ ! -e ${destination} || ( -e ${destination} && ${redownloadInfra} == "true" ) ]]; then
        ...
        (curl -u "${M2_SETTINGS_REPO_USERNAME}:${M2_SETTINGS_REPO_PASSWORD}" "${pathToJar}" "${verifySslFlag}" -o "${destination}" --fail && echo "File downloaded successfully!") || (echo "Failed to download file!" && return 1)
    else
...
function retrieveCurrentVersion() {
    if [[ "${PROJECT_TYPE}" == "GRADLE" ]]; then
        ...
        local result=$( ruby -r rexml/document -e 'puts REXML::Document.new(File.new(ARGV.shift)).elements["/project/artifactId"].text' pom.xml || ./mvnw ${MAVEN_ARGS} org.apache.maven.plugins:maven-help-plugin:2.2:evaluate -Dexpression=project.version |grep -Ev '(^\[|Download\w+:)' )
        result=$( echo "${result}" | tail -1 )
        echo "${result}"
    fi
}
